...
function rateLimit(req) {
  ...
  if (recent.length >= 5) fail(429, 'Too many contact requests. Please try again later.');
...
exports.create = async (req,res) => {
  if (String(req.body.website || '').trim()) return res.status(201).json({message:'Thank you! Your request has been submitted successfully. Our team will contact you soon.'});
  rateLimit(req);
  ...
  const result = await withWrite(tx => tx.run(`INSERT INTO contact_inquiries(name,email,phone,organization,service,message)
    VALUES (?,?,?,'','General enquiry',?) RETURNING id`,name,email,phone,message));
  req.io?.to('superadmin').emit('contactInquiry',{id:result.lastID});
  res.status(201).json({message:'Thank you! Your request has been submitted successfully. Our team will contact you soon.',inquiryId:result.lastID});
};
...
exports.setStatus = async (req,res) => {
  ...
  if (!Number.isInteger(id) || id <= 0) fail(404,'uiry not found.');
  ...
  if (!result.changes) fail(404,'uiry not found.');
  req.io?.to('superadmin').emit('contactInquiryUpdated',{id,status});
  res.json({message:status==='resolved'?'Inquiry marked resolved.':status==='read'?'Inquiry marked read.':'Inquiry moved back to unread.'});
};
...
exports.markAllRead = async (req,res) => {
  ...
  res.json({message:'All contact notifications marked read.',updated:result.changes});
};
